...
namespace CDP4MessagePackSerializer
{
    ...
    public class PossibleFiniteStateListMessagePackFormatter : IMessagePackFormatter<PossibleFiniteStateList>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, PossibleFiniteStateList possibleFiniteStateList, MessagePackSerializerOptions options)
        {
            ...
            writer.WriteArrayHeader(possibleFiniteStateList.Attachment.Count);
            foreach (var identifier in possibleFiniteStateList.Attachment.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            {
                writer.Write(possibleFiniteStateList.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 16:
                        if (reader.TryReadNil())
                        {
                            possibleFiniteStateList.Actor = null;
                        }
                        else
                        {
                            possibleFiniteStateList.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
