// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SolutionMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class SolutionMessagePackFormatter : IMessagePackFormatter<Solution>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, Solution solution, MessagePackSerializerOptions options)
        {
            ...
            foreach (var identifier in solution.ExcludedDomain.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            writer.WriteArrayHeader(solution.ExcludedPerson.Count);
            foreach (var identifier in solution.ExcludedPerson.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            {
                writer.Write(solution.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 11:
                        ...
                        {
                            solution.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
