// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RuleViolationMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class RuleViolationMessagePackFormatter : IMessagePackFormatter<RuleViolation>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, RuleViolation ruleViolation, MessagePackSerializerOptions options)
        {
            ...
            foreach (var identifier in ruleViolation.ExcludedDomain.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            writer.WriteArrayHeader(ruleViolation.ExcludedPerson.Count);
            foreach (var identifier in ruleViolation.ExcludedPerson.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            {
                writer.Write(ruleViolation.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 8:
                        if (reader.TryReadNil())
                        {
                            ruleViolation.Actor = null;
                        }
                        else
                        {
                            ruleViolation.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
