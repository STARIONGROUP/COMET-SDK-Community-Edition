// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ScaleReferenceQuantityValueMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class ScaleReferenceQuantityValueMessagePackFormatter : IMessagePackFormatter<ScaleReferenceQuantityValue>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, ScaleReferenceQuantityValue scaleReferenceQuantityValue, MessagePackSerializerOptions options)
        {
            ...
            foreach (var identifier in scaleReferenceQuantityValue.ExcludedDomain.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            writer.WriteArrayHeader(scaleReferenceQuantityValue.ExcludedPerson.Count);
            foreach (var identifier in scaleReferenceQuantityValue.ExcludedPerson.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            {
                writer.Write(scaleReferenceQuantityValue.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 8:
                        if (reader.TryReadNil())
                        {
                            scaleReferenceQuantityValue.Actor = null;
                        }
                        else
                        {
                            scaleReferenceQuantityValue.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
