// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SiteLogEntryMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class SiteLogEntryMessagePackFormatter : IMessagePackFormatter<SiteLogEntry>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, SiteLogEntry siteLogEntry, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(siteLogEntry.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 15:
                        if (reader.TryReadNil())
                        {
                            siteLogEntry.Actor = null;
                        }
                        else
                        {
                            siteLogEntry.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
