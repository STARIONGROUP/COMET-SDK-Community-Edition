// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParameterGroupMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class ParameterGroupMessagePackFormatter : IMessagePackFormatter<ParameterGroup>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, ParameterGroup parameterGroup, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(parameterGroup.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 8:
                        if (reader.TryReadNil())
                        {
                            parameterGroup.Actor = null;
                        }
                        else
                        {
                            parameterGroup.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
