...
namespace CDP4MessagePackSerializer
{
    ...
    public class ValueGroupMessagePackFormatter : IMessagePackFormatter<ValueGroup>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, ValueGroup valueGroup, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(valueGroup.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            }
            writer.WriteArrayHeader(valueGroup.Attachment.Count);
            foreach (var identifier in valueGroup.Attachment.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
