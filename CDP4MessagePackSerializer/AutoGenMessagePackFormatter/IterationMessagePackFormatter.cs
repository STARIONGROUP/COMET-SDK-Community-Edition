// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IterationMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class IterationMessagePackFormatter : IMessagePackFormatter<Iteration>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, Iteration iteration, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(iteration.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 27:
                        if (reader.TryReadNil())
                        {
                            iteration.Actor = null;
                        }
                        else
                        {
                            iteration.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
