// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IterationSetupMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class IterationSetupMessagePackFormatter : IMessagePackFormatter<IterationSetup>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, IterationSetup iterationSetup, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(iterationSetup.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 13:
                        if (reader.TryReadNil())
                        {
                            iterationSetup.Actor = null;
                        }
                        else
                        {
                            iterationSetup.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
