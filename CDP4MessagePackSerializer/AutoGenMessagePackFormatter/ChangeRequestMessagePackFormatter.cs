// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ChangeRequestMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class ChangeRequestMessagePackFormatter : IMessagePackFormatter<ChangeRequest>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, ChangeRequest changeRequest, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(changeRequest.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 21:
                        if (reader.TryReadNil())
                        {
                            changeRequest.Actor = null;
                        }
                        else
                        {
                            changeRequest.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
