// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TextualNoteMessagePackFormatter.cs" company="Starion Group S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class TextualNoteMessagePackFormatter : IMessagePackFormatter<TextualNote>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, TextualNote textualNote, MessagePackSerializerOptions options)
        {
            ...
            {
                writer.Write(textualNote.Actor.Value.ToByteArray());
            }
            else
            {
                writer.WriteNil();
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 13:
                        if (reader.TryReadNil())
                        {
                            textualNote.Actor = null;
                        }
                        else
                        {
                            textualNote.Actor = reader.ReadBytes().ToGuid();
                        }
                        break;
                    default:
                        reader.Skip();
