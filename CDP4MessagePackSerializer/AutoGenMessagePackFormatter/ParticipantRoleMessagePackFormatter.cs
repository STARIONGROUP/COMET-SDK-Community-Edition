// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParticipantRoleMessagePackFormatter.cs" company="RHEA System S.A.">
...
/* -------------------------------------------- | ---------------------------- | ----------- | ------- *
 ...
 | 8     | participantPermission                | Guid                         | 0..*        |  1.0.0  |
...
namespace CDP4MessagePackSerializer
{
    ...
    public class ParticipantRoleMessagePackFormatter : IMessagePackFormatter<ParticipantRole>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, ParticipantRole participantRole, MessagePackSerializerOptions options)
        {
            ...
            writer.WriteArrayHeader(participantRole.Attachment.Count);
            foreach (var identifier in participantRole.Attachment.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 3:
                        ...
                        for (valueCounter = 0; valueCounter < valueLength; valueCounter++)
                        {
                            participantRole.Attachment.Add(reader.ReadBytes().ToGuid());
                        }
                        break;
                    case 4:
                        valueLength = reader.ReadArrayHeader();
