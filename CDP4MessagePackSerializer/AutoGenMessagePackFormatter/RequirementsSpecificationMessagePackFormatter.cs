// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RequirementsSpecificationMessagePackFormatter.cs" company="RHEA System S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class RequirementsSpecificationMessagePackFormatter : IMessagePackFormatter<RequirementsSpecification>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, RequirementsSpecification requirementsSpecification, MessagePackSerializerOptions options)
        {
            ...
            writer.WriteArrayHeader(requirementsSpecification.Attachment.Count);
            foreach (var identifier in requirementsSpecification.Attachment.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 3:
                        ...
                        for (valueCounter = 0; valueCounter < valueLength; valueCounter++)
                        {
                            requirementsSpecification.Attachment.Add(reader.ReadBytes().ToGuid());
                        }
                        break;
                    case 4:
                        valueLength = reader.ReadArrayHeader();
