...
namespace CDP4MessagePackSerializer
{
    ...
    public class StakeholderValueMessagePackFormatter : IMessagePackFormatter<StakeholderValue>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, StakeholderValue stakeholderValue, MessagePackSerializerOptions options)
        {
            ...
            writer.WriteArrayHeader(stakeholderValue.Attachment.Count);
            foreach (var identifier in stakeholderValue.Attachment.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 13:
                        ...
                        for (valueCounter = 0; valueCounter < valueLength; valueCounter++)
                        {
                            stakeholderValue.Attachment.Add(reader.ReadBytes().ToGuid());
                        }
                        break;
                    default:
                        reader.Skip();
