// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TimeOfDayParameterTypeMessagePackFormatter.cs" company="RHEA System S.A.">
...
namespace CDP4MessagePackSerializer
{
    ...
    public class TimeOfDayParameterTypeMessagePackFormatter : IMessagePackFormatter<TimeOfDayParameterType>
    {
        ...
        public void Serialize(ref MessagePackWriter writer, TimeOfDayParameterType timeOfDayParameterType, MessagePackSerializerOptions options)
        {
            ...
            writer.WriteArrayHeader(timeOfDayParameterType.Attachment.Count);
            foreach (var identifier in timeOfDayParameterType.Attachment.OrderBy(x => x, guidComparer))
            {
                writer.Write(identifier.ToByteArray());
            }
            ...
            for (var i = 0; i < propertyCounter; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 3:
                        ...
                        for (valueCounter = 0; valueCounter < valueLength; valueCounter++)
                        {
                            timeOfDayParameterType.Attachment.Add(reader.ReadBytes().ToGuid());
                        }
                        break;
                    case 4:
                        valueLength = reader.ReadArrayHeader();
