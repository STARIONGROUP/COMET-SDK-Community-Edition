// <copyright file="RelationalExpressionRuleChecker.cs" company="Starion Group S.A.">
...
namespace CDP4Rules.RuleCheckers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="RelationalExpressionRuleChecker"/> is to execute the rules for instances of type <see cref="RelationalExpression"/>
    /// </summary>
    [RuleChecker(typeof(RelationalExpression))]
    public class RelationalExpressionRuleChecker : RuleChecker
    {
        /// <summary>
        /// Checks whether a referenced <see cref="IDeprecatableThing"/> is deprecated
        /// </summary>
        /// <param name="thing">
        /// The subject <see cref="RelationalExpression"/>
        /// </param>
        /// <returns>
        ...
        /// thrown when <paramref name="thing"/> is not an <see cref="RelationalExpression"/>
        /// </exception>
        [Rule("MA-0500")]
        public IEnumerable<RuleCheckResult> ChecksWhetherAReferencedDeprecatableThingIsDeprecated(Thing thing)
        {
            var relationalExpression = VerifyThingArgument(thing);
        ...
        public IEnumerable<RuleCheckResult> CheckWhetherReferencedParameterTypeIsInChainOfRdls(Thing thing)
        {
            ...
            if (!modelReferenceDataLibrary.IsParameterTypeInChainOfRdls(relationalExpression.ParameterType))
            {
                var result = new RuleCheckResult(thing, rule.Id, $"The referenced ParameterType {relationalExpression.ParameterType.Iid}:{relationalExpression.ParameterType.ShortName} is not in the chain of Reference Data Libraries", SeverityKind.Error);
                results.Add(result);
            }
        ...
        /// </param>
        /// <returns>
        /// an instance of <see cref="RelationalExpression"/>
        /// </returns>
        /// <exception cref="ArgumentNullException">
        /// thrown when <paramref name="thing"/> is null
        /// </exception>
        ...
        /// thrown when <paramref name="thing"/> is not an <see cref="RelationalExpression"/>
        /// </exception>
        private static RelationalExpression VerifyThingArgument(Thing thing)
        {
            if (thing == null)
            {
            ...
            {
                throw new ArgumentException($"{nameof(thing)} with Iid:{thing.Iid} is not an RelationalExpression", nameof(thing));
            }
