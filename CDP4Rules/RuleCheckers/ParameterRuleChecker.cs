// <copyright file="ParameterRuleChecker.cs" company="Starion Group S.A.">
...
namespace CDP4Rules.RuleCheckers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ParameterRuleChecker"/> is to execute the rules for instances of type <see cref="Parameter"/>
    /// </summary>
    [RuleChecker(typeof(Parameter))]
    public class ParameterRuleChecker : RuleChecker
    {
        /// <summary>
        ...
        /// thrown when <paramref name="thing"/> is not an <see cref="Parameter"/>
        /// </exception>
        private static Parameter VerifyThingArgument(Thing thing)
        {
            if (thing == null)
            {
            ...
            if (parameter == null)
            {
                throw new ArgumentException($"{nameof(thing)} with Iid:{thing.Iid} is not an Parameter", nameof(thing));
            }
