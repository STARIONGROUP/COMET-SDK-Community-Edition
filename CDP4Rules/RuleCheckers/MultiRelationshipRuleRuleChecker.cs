// <copyright file="MultiRelationshipRuleRuleChecker.cs" company="Starion Group S.A.">
...
namespace CDP4Rules.RuleCheckers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="MultiRelationshipRuleRuleChecker"/> is to execute the rules for instances of type <see cref="MultiRelationshipRule"/>
    /// </summary>
    [RuleChecker(typeof(MultiRelationshipRule))]
    public class MultiRelationshipRuleRuleChecker : RuleChecker
    {
        /// <summary>
        ...
        public IEnumerable<RuleCheckResult> CheckWhetherReferencedCategoryIsInChainOfRdls(Thing thing)
        {
            ...
            var referenceDataLibrary = (ReferenceDataLibrary)multiRelationshipRule.Container;

            if (!referenceDataLibrary.IsCategoryInChainOfRdls(multiRelationshipRule.RelationshipCategory))
            {
                var result = new RuleCheckResult(thing, rule.Id, $"The MultiRelationshipRule.RelationshipCategory {multiRelationshipRule.RelationshipCategory.Iid}:{multiRelationshipRule.RelationshipCategory.ShortName} of {multiRelationshipRule.Iid}:{multiRelationshipRule.ShortName} is not in the chain of Reference Data Libraries", SeverityKind.Error);
                results.Add(result);
            }
        ...
        private MultiRelationshipRule VerifyThingArgument(Thing thing)
        {
            ...
            {
                throw new ArgumentException($"{nameof(thing)} with Iid:{thing.Iid} is not an MultiRelationshipRule", nameof(thing));
            }
