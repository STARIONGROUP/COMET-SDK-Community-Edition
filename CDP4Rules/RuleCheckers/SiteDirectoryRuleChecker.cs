// <copyright file="SiteDirectoryRuleChecker.cs" company="Starion Group S.A.">
...
namespace CDP4Rules.RuleCheckers
{
    ...
    public class SiteDirectoryRuleChecker : RuleChecker
    {
        ...
        {
            var siteDirectory = VerifyThingArgument(thing);
        ...
        /// thrown when <paramref name="thing"/> is not an <see cref="SiteDirectory"/>
        /// </exception>
        private static SiteDirectory VerifyThingArgument(Thing thing)
        {
            if (thing == null)
            {
            ...
            {
                throw new ArgumentException($"{nameof(thing)} with Iid:{thing.Iid} is not a SiteDirectory", nameof(thing));
            }
