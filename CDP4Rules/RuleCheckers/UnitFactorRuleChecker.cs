// <copyright file="UnitFactorRuleChecker.cs" company="Starion Group S.A.">
...
namespace CDP4Rules.RuleCheckers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="UnitFactorRuleChecker"/> is to execute the rules for instances of type <see cref="UnitFactor"/>
    /// </summary>
    [RuleChecker(typeof(UnitFactor))]
    public class UnitFactorRuleChecker : RuleChecker
    {
        /// <summary>
        /// Checks whether a referenced <see cref="IDeprecatableThing"/> is deprecated
        /// </summary>
        /// <param name="thing">
        /// The subject <see cref="UnitFactor"/>
        /// </param>
        /// <returns>
        ...
        /// thrown when <paramref name="thing"/> is not an <see cref="UnitFactor"/>
        /// </exception>
        [Rule("MA-0500")]
        public IEnumerable<RuleCheckResult> ChecksWhetherAReferencedDeprecatableThingIsDeprecated(Thing thing)
        {
            var unitFactor = VerifyThingArgument(thing);
        ...
        public IEnumerable<RuleCheckResult> CheckWhetherReferencedMeasurementUnitIsInChainOfRdls(Thing thing)
        {
            ...
            if (!referenceDataLibrary.IsMeasurementUnitInChainOfRdls(unitFactor.Unit))
            {
                var result = new RuleCheckResult(thing, rule.Id, $"The referenced MeasurementUnit {unitFactor.Unit.Iid}:{unitFactor.Unit.ShortName} of UnitFactor.Unit is not in the chain of Reference Data Libraries", SeverityKind.Error);
                results.Add(result);
            }
        ...
        /// </param>
        /// <returns>
        /// an instance of <see cref="UnitFactor"/>
        /// </returns>
        /// <exception cref="ArgumentNullException">
        /// thrown when <paramref name="thing"/> is null
        /// </exception>
        ...
        /// thrown when <paramref name="thing"/> is not an <see cref="UnitFactor"/>
        /// </exception>
        private static UnitFactor VerifyThingArgument(Thing thing)
        {
            if (thing == null)
            {
            ...
            if (unitFactor == null)
            {
                throw new ArgumentException($"{nameof(thing)} with Iid:{thing.Iid} is not a UnitFactor", nameof(thing));
            }
