// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PocoThingFactoryTestFixture.cs" company="Starion Group S.A.">
...
namespace CDP4Common.NetCore.Tests.Helpers
{
    ...
    public class PocoThingFactoryTestFixture
    {
        ...
        public void Verify_that_ResolveList_works()
        {
            ...
            Assert.That(this.orderedItems.OrderBy(x => x.K).Select(x => x.V), Is.EqualTo(this.possibleFiniteStateList.PossibleState.Select(x => x.Iid)));
        }
