            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "PossibleState":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.PossibleState.Remove(toBeRemoved);
                        } 
                        if (!this.PossibleState.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from PossibleState property results in inconsistent ActualFiniteState.");
                            result = false;
                        }
                        break;
                }
