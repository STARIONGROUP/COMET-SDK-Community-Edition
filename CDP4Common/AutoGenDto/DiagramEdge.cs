// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DiagramEdge.cs" company="Starion Group S.A.">
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Source":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Source property results in inconsistent DiagramEdge.");
                            result = false;
                        }
                        break;
                    ...
                    case "Target":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Target property results in inconsistent DiagramEdge.");
                            result = false;
                        }
                        break;
                }
