// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Solution.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="Solution"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class Solution : GenericAnnotation, IOwnedThing
    {
        ...
        /// The revision number.
        /// </param>
        public Solution(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent Solution.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent Solution.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.Solution"/> from a <see cref="Solution"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.Solution"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.Solution(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as Solution;
            if (original == null)
            {
