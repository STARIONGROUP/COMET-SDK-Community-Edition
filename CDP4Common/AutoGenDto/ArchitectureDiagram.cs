...
namespace CDP4Common.DTO
{
    ...
    public partial class ArchitectureDiagram : DiagramCanvas, IOwnedThing
    {
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            {
                dictionary.Add("TopArchitectureElement", new [] { this.TopArchitectureElement.Value });
            }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent ArchitectureDiagram.");
                                    result = false;
                                }
                                break;

                            case "TopArchitectureElement":
                                this.TopArchitectureElement = null;
                                break;
                        }
                    }
                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ArchitectureDiagram.");
                            result = false;
                        }
                        break;
