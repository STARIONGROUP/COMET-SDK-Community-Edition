...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="Category"/> class.
    /// </summary>
    [DataContract]
    ...
    public partial class Category : DefinedThing, IDeprecatableThing
    {
        ...
        public Category()
        {
            ...
            this.SuperCategory = new List<Guid>();
        }
        ...
        /// The revision number.
        /// </param>
        public Category(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
            this.PermissibleClass = new List<ClassKind>();
            this.SuperCategory = new List<Guid>();
        }
        ...
        /// <summary>
        /// Gets or sets the list of unique identifiers of the referenced SuperCategory instances.
        /// </summary>
        [UmlInformation(aggregation: AggregationKind.None, isDerived: false, isOrdered: false, isNullable: false, isPersistent: true)]
        [DataMember]
        public List<Guid> SuperCategory { get; set; }
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("SuperCategory", this.SuperCategory);
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "SuperCategory":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.SuperCategory.Remove(toBeRemoved);
                        } 
                        break;
                }
            }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.SiteDirectoryData.Category"/> from a <see cref="Category"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.SiteDirectoryData.Category"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.SiteDirectoryData.Category(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as Category;
            if (original == null)
            {
            ...
            this.PermissibleClass = original.PermissibleClass;

            this.ShortName = original.ShortName;

            foreach (var guid in original.SuperCategory)
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == guid);
                this.SuperCategory.Add(copy.Value == null ? guid : copy.Value.Iid);
            }
        ...
        public override bool ResolveCopyReference(IReadOnlyDictionary<Thing, Thing> originalCopyMap)
        {
            ...
            for (var i = 0; i < this.SuperCategory.Count; i++)
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == this.SuperCategory[i]);
                if (copy.Key != null)
                {
                    this.SuperCategory[i] = copy.Value.Iid;
                    hasChanges = true;
                }
            }
