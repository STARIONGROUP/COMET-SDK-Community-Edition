// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParametricConstraint.cs" company="Starion Group S.A.">
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Expression":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.Expression.Remove(toBeRemoved);
                        } 
                        if (!this.Expression.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Expression property results in inconsistent ParametricConstraint.");
                            result = false;
                        }
                        break;
