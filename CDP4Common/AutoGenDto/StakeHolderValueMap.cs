// --------------------------------------------------------------------------------------------------------------------
// <copyright file="StakeHolderValueMap.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    public partial class StakeHolderValueMap : DefinedThing, ICategorizableThing
    {
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("Settings", this.Settings);
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Settings":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.Settings.Remove(toBeRemoved);
                        } 
                        if (!this.Settings.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Settings property results in inconsistent StakeHolderValueMap.");
                            result = false;
                        }
                        break;
