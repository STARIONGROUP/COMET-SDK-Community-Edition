// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ModelLogEntry.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="ModelLogEntry"/> class.
    /// </summary>
    [DataContract]
    ...
    public partial class ModelLogEntry : Thing, IAnnotation, ICategorizableThing, ILogEntry, ITimeStampedThing
    {
        ...
        /// The revision number.
        /// </param>
        public ModelLogEntry(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
            this.AffectedDomainIid = new List<Guid>();
            this.AffectedItemIid = new List<Guid>();
            this.Category = new List<Guid>();
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("LogEntryChangelogItem", this.LogEntryChangelogItem);
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "LogEntryChangelogItem":
                                this.LogEntryChangelogItem.Remove(id);
                                break;
                        }
                    }
                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "LogEntryChangelogItem":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.LogEntryChangelogItem.Remove(toBeRemoved);
                        } 
                        break;
                }
            }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.EngineeringModelData.ModelLogEntry"/> from a <see cref="ModelLogEntry"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.EngineeringModelData.ModelLogEntry"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.EngineeringModelData.ModelLogEntry(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as ModelLogEntry;
            if (original == null)
            {
            ...
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == guid);
                this.AffectedItemIid.Add(copy.Value == null ? guid : copy.Value.Iid);
            }
