// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RequestForWaiver.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="RequestForWaiver"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    [Container(typeof(EngineeringModel), "ModellingAnnotation")]
    public partial class RequestForWaiver : ContractDeviation
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="RequestForWaiver"/> class.
        /// </summary>
        ...
        /// The revision number.
        /// </param>
        public RequestForWaiver(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }

        /// <summary>
        /// Gets the route for the current <see ref="RequestForWaiver"/>.
        /// </summary>
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Author":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Author property results in inconsistent RequestForWaiver.");
                                    result = false;
                                }
                                break;
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent RequestForWaiver.");
                                    result = false;
                                }
                                break;
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent RequestForWaiver.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent RequestForWaiver.");
                            result = false;
                        }
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.RequestForWaiver"/> from a <see cref="RequestForWaiver"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.RequestForWaiver"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.RequestForWaiver(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as RequestForWaiver;
            if (original == null)
            {
