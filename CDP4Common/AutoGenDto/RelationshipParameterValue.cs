...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="RelationshipParameterValue"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class RelationshipParameterValue : ParameterValue
    {
        ...
        /// The revision number.
        /// </param>
        public RelationshipParameterValue(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "ParameterType":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from ParameterType property results in inconsistent RelationshipParameterValue.");
                                    result = false;
                                }
                                break;
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ParameterType":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ParameterType property results in inconsistent RelationshipParameterValue.");
                            result = false;
                        }
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.EngineeringModelData.RelationshipParameterValue"/> from a <see cref="RelationshipParameterValue"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.EngineeringModelData.RelationshipParameterValue"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.EngineeringModelData.RelationshipParameterValue(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as RelationshipParameterValue;
            if (original == null)
            {
