            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ReferencedCategory":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ReferencedCategory.Remove(toBeRemoved);
                        } 
                        if (!this.ReferencedCategory.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ReferencedCategory property results in inconsistent ReferencerRule.");
                            result = false;
                        }
                        break;
                    ...
                    case "ReferencingCategory":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ReferencingCategory property results in inconsistent ReferencerRule.");
                            result = false;
                        }
                        break;
                }
