...
namespace CDP4Common.DTO
{
    ...
    public partial class Attachment : Thing
    {
        /// <summary>
                ...
                if (foundProperty.Any())
                {
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "FileType":
                                if (addModelErrors && this.FileType.Count == 1)
                                {
                                    errors.Add($"Removing reference '{id}' from FileType property results in inconsistent Attachment.");
                                    result = false;
                                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "FileType":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from FileType property results in inconsistent Attachment.");
                            result = false;
                        }
                        break;
                }
