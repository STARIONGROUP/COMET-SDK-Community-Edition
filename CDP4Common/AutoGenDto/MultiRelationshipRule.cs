...
namespace CDP4Common.DTO
{
    ...
    public partial class MultiRelationshipRule : Rule
    {
        /// <summary>
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "RelatedCategory":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.RelatedCategory.Remove(toBeRemoved);
                        } 
                        if (!this.RelatedCategory.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from RelatedCategory property results in inconsistent MultiRelationshipRule.");
                            result = false;
                        }
                        break;
                    ...
                    case "RelationshipCategory":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from RelationshipCategory property results in inconsistent MultiRelationshipRule.");
                            result = false;
                        }
                        break;
                }
