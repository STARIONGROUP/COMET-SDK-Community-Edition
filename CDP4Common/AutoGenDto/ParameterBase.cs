// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParameterBase.cs" company="Starion Group S.A.">
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ParameterBase.");
                            result = false;
                        }
                        break;
                    ...
                    case "ParameterType":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ParameterType property results in inconsistent ParameterBase.");
                            result = false;
                        }
                        break;
