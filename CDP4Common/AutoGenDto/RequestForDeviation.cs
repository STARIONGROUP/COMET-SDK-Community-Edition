// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RequestForDeviation.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="RequestForDeviation"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    [Container(typeof(EngineeringModel), "ModellingAnnotation")]
    public partial class RequestForDeviation : ContractDeviation
    {
        /// <summary>
        ...
        /// The revision number.
        /// </param>
        public RequestForDeviation(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }

        /// <summary>
        /// Gets the route for the current <see ref="RequestForDeviation"/>.
        /// </summary>
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Author":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Author property results in inconsistent RequestForDeviation.");
                                    result = false;
                                }
                                break;
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent RequestForDeviation.");
                                    result = false;
                                }
                                break;
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent RequestForDeviation.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent RequestForDeviation.");
                            result = false;
                        }
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.RequestForDeviation"/> from a <see cref="RequestForDeviation"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.RequestForDeviation"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.RequestForDeviation(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as RequestForDeviation;
            if (original == null)
            {
