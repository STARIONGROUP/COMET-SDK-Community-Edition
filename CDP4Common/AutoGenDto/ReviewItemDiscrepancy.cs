// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ReviewItemDiscrepancy.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    public partial class ReviewItemDiscrepancy : ModellingAnnotationItem
    {
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("Solution", this.Solution);
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent ReviewItemDiscrepancy.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ReviewItemDiscrepancy.");
                            result = false;
                        }
                        break;
                    ...
                    case "Solution":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.Solution.Remove(toBeRemoved);
                        } 
                        break;
