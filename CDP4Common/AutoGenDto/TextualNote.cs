// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TextualNote.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="TextualNote"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class TextualNote : Note
    {
        ...
        /// The revision number.
        /// </param>
        public TextualNote(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
        ...
        /// <summary>
        /// Gets the route for the current <see ref="TextualNote"/>.
        /// </summary>
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent TextualNote.");
                                    result = false;
                                }
                                break;
                        }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Owner":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent TextualNote.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.TextualNote"/> from a <see cref="TextualNote"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.TextualNote"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.TextualNote(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as TextualNote;
            if (original == null)
            {
