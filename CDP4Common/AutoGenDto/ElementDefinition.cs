...
namespace CDP4Common.DTO
{
    ...
    public partial class ElementDefinition : ElementBase
    {
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("ContainedElement", this.ContainedElement);
            ...
            dictionary.Add("Parameter", this.Parameter);

            dictionary.Add("ParameterGroup", this.ParameterGroup);

            dictionary.Add("ReferencedElement", this.ReferencedElement);
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "ContainedElement":
                                this.ContainedElement.Remove(id);
                                break;
                            ...
                            case "ParameterGroup":
                                this.ParameterGroup.Remove(id);
                                break;

                            case "ReferencedElement":
                                this.ReferencedElement.Remove(id);
                                break;
                        }
                    }
                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ContainedElement":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ContainedElement.Remove(toBeRemoved);
                        } 
                        break;
                    ...
                    case "Owner":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ElementDefinition.");
                            result = false;
                        }
                        break;
                    ...
                    case "Parameter":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.Parameter.Remove(toBeRemoved);
                        } 
                        break;
                    ...
                    case "ParameterGroup":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ParameterGroup.Remove(toBeRemoved);
                        } 
                        break;
                    ...
                    case "ReferencedElement":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ReferencedElement.Remove(toBeRemoved);
                        } 
                        break;
                }
            }
