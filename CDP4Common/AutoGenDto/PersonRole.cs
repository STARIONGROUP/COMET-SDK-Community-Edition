...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="PersonRole"/> class.
    /// </summary>
    [DataContract]
    ...
    public partial class PersonRole : DefinedThing, IDeprecatableThing
    {
        ...
        public PersonRole()
        {
            this.PersonPermission = new List<Guid>();
        }
        ...
        /// The revision number.
        /// </param>
        public PersonRole(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
            this.PersonPermission = new List<Guid>();
        }
        ...
        /// <summary>
        /// Gets or sets the unique identifiers of the contained PersonPermission instances.
        /// </summary>
        [UmlInformation(aggregation: AggregationKind.Composite, isDerived: false, isOrdered: false, isNullable: false, isPersistent: true)]
        [DataMember]
        public List<Guid> PersonPermission { get; set; }
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("PersonPermission", this.PersonPermission);
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "PersonPermission":
                                this.PersonPermission.Remove(id);
                                break;
                        }
                    }
                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "PersonPermission":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.PersonPermission.Remove(toBeRemoved);
                        } 
                        break;
                }
            }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.SiteDirectoryData.PersonRole"/> from a <see cref="PersonRole"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.SiteDirectoryData.PersonRole"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.SiteDirectoryData.PersonRole(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as PersonRole;
            if (original == null)
            {
            ...
            foreach (var guid in original.PersonPermission)
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == guid);
                if (Equals(copy, default(KeyValuePair<Thing, Thing>)))
                {
                ...
                this.PersonPermission.Add(copy.Value.Iid);
            }
