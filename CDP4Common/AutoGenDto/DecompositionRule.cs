            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ContainedCategory":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ContainedCategory.Remove(toBeRemoved);
                        } 
                        if (!this.ContainedCategory.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ContainedCategory property results in inconsistent DecompositionRule.");
                            result = false;
                        }
                        break;
                    ...
                    case "ContainingCategory":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ContainingCategory property results in inconsistent DecompositionRule.");
                            result = false;
                        }
                        break;
