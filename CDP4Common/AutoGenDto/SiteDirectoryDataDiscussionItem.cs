// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SiteDirectoryDataDiscussionItem.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="SiteDirectoryDataDiscussionItem"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class SiteDirectoryDataDiscussionItem : DiscussionItem
    {
        ...
        /// The revision number.
        /// </param>
        public SiteDirectoryDataDiscussionItem(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            case "Author":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Author property results in inconsistent SiteDirectoryDataDiscussionItem.");
                                    result = false;
                                }
                                break;
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent SiteDirectoryDataDiscussionItem.");
                            result = false;
                        }
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.SiteDirectoryDataDiscussionItem"/> from a <see cref="SiteDirectoryDataDiscussionItem"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.SiteDirectoryDataDiscussionItem"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.SiteDirectoryDataDiscussionItem(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as SiteDirectoryDataDiscussionItem;
            if (original == null)
            {
