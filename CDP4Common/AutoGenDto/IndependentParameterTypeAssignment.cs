// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IndependentParameterTypeAssignment.cs" company="Starion Group S.A.">
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "ParameterType":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from ParameterType property results in inconsistent IndependentParameterTypeAssignment.");
                                    result = false;
                                }
                                break;
                        }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ParameterType":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ParameterType property results in inconsistent IndependentParameterTypeAssignment.");
                            result = false;
                        }
                        break;
                }
