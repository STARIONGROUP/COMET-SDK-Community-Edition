// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DomainFileStore.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="DomainFileStore"/> class.
    /// </summary>
    [DataContract]
    ...
    public partial class DomainFileStore : FileStore
    {
        ...
        /// The revision number.
        /// </param>
        public DomainFileStore(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent DomainFileStore.");
                                    result = false;
                                }
                                break;
                        }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Owner":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent DomainFileStore.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.EngineeringModelData.DomainFileStore"/> from a <see cref="DomainFileStore"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.EngineeringModelData.DomainFileStore"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.EngineeringModelData.DomainFileStore(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as DomainFileStore;
            if (original == null)
            {
            ...
            foreach (var guid in original.File)
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == guid);
                ...
                this.File.Add(copy.Value.Iid);
            }

            foreach (var guid in original.Folder)
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == guid);
                if (Equals(copy, default(KeyValuePair<Thing, Thing>)))
                {
                ...
                this.Folder.Add(copy.Value.Iid);
            }
