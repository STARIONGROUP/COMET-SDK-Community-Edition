// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Approval.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="Approval"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class Approval : GenericAnnotation, IOwnedThing
    {
        ...
        /// The revision number.
        /// </param>
        public Approval(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
        ...
        [UmlInformation(aggregation: AggregationKind.None, isDerived: false, isOrdered: false, isNullable: false, isPersistent: true)]
        [DataMember]
        public AnnotationApprovalKind Classification { get; set; }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent Approval.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent Approval.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.Approval"/> from a <see cref="Approval"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.Approval"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.Approval(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as Approval;
            if (original == null)
            {
