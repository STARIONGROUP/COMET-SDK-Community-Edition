// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DiagramObject.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="DiagramObject"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class DiagramObject : DiagramShape
    {
        ...
        /// The revision number.
        /// </param>
        public DiagramObject(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("DiagramElement", this.DiagramElement);
                ...
                if (foundProperty.Any())
                {
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "DepictedThing":
                                this.DepictedThing = null;
                                break;

                            case "DiagramElement":
                                this.DiagramElement.Remove(id);
                                break;
                            ...
                            case "LocalStyle":
                                this.LocalStyle.Remove(id);
                                break;

                            case "SharedStyle":
                                this.SharedStyle = null;
                                break;
                        }
                    }
                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    case "Bounds":
                        ...
                        {
                            this.Bounds.Remove(toBeRemoved);
                        } 
                        break;
                    ...
                    case "DiagramElement":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.DiagramElement.Remove(toBeRemoved);
                        } 
                        break;
                    ...
                    case "LocalStyle":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.LocalStyle.Remove(toBeRemoved);
                        } 
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.DiagramData.DiagramObject"/> from a <see cref="DiagramObject"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.DiagramData.DiagramObject"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.DiagramData.DiagramObject(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as DiagramObject;
            if (original == null)
            {
