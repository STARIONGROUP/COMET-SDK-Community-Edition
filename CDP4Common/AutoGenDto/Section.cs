// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Section.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="Section"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class Section : Thing, ICategorizableThing, INamedThing, IOwnedThing, IShortNamedThing, ITimeStampedThing
    {
        ...
        public Section()
        {
            ...
            this.Page = new List<OrderedItem>();
        }
        ...
        /// The revision number.
        /// </param>
        public Section(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
            this.Category = new List<Guid>();
            this.Page = new List<OrderedItem>();
        }
        ...
        /// <summary>
        /// Gets or sets the unique identifiers of the contained Page instances.
        /// </summary>
        [UmlInformation(aggregation: AggregationKind.Composite, isDerived: false, isOrdered: true, isNullable: false, isPersistent: true)]
        [DataMember]
        public List<OrderedItem> Page { get; set; }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent Section.");
                                    result = false;
                                }
                                break;
                        }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Owner":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent Section.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.Section"/> from a <see cref="Section"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.Section"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.Section(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as Section;
            if (original == null)
            {
            ...
            foreach (var orderedItem in original.Page)
            {
