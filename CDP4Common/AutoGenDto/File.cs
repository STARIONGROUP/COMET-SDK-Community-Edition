...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="File"/> class.
    /// </summary>
    [DataContract]
    ...
    public partial class File : Thing, ICategorizableThing, IOwnedThing
    {
        ...
        public File()
        {
            ...
            this.FileRevision = new List<Guid>();
        }
        ...
        /// The revision number.
        /// </param>
        public File(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
            this.Category = new List<Guid>();
            this.FileRevision = new List<Guid>();
        }
        ...
        /// <summary>
        /// Gets or sets the unique identifiers of the contained FileRevision instances.
        /// </summary>
        [UmlInformation(aggregation: AggregationKind.Composite, isDerived: false, isOrdered: false, isNullable: false, isPersistent: true)]
        [DataMember]
        public List<Guid> FileRevision { get; set; }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "FileRevision":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.FileRevision.Remove(toBeRemoved);
                        } 
                        if (!this.FileRevision.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from FileRevision property results in inconsistent File.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent File.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.EngineeringModelData.File"/> from a <see cref="File"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.EngineeringModelData.File"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.EngineeringModelData.File(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as File;
            if (original == null)
            {
            ...
            foreach (var guid in original.FileRevision)
            {
                var copy = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == guid);
                ...
                this.FileRevision.Add(copy.Value.Iid);
            }
