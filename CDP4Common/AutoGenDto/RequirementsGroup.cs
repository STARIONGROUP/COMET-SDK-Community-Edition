...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="RequirementsGroup"/> class.
    /// </summary>
    [DataContract]
    ...
    public partial class RequirementsGroup : RequirementsContainer
    {
        ...
        /// The revision number.
        /// </param>
        public RequirementsGroup(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent RequirementsGroup.");
                                    result = false;
                                }
                                break;
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Owner":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent RequirementsGroup.");
                            result = false;
                        }
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.EngineeringModelData.RequirementsGroup"/> from a <see cref="RequirementsGroup"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.EngineeringModelData.RequirementsGroup"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.EngineeringModelData.RequirementsGroup(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as RequirementsGroup;
            if (original == null)
            {
