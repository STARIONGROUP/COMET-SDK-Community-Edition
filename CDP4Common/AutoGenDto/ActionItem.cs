// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ActionItem.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    public partial class ActionItem : ModellingAnnotationItem
    {
        /// <summary>
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    case "Actionee":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Actionee property results in inconsistent ActionItem.");
                            result = false;
                        }
                        break;
                    ...
                    case "Author":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent ActionItem.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ActionItem.");
                            result = false;
                        }
                        break;
