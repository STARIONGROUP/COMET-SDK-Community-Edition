// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ThingReference.cs" company="Starion Group S.A.">
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "ReferencedThing":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from ReferencedThing property results in inconsistent ThingReference.");
                                    result = false;
                                }
                                break;
                        }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ReferencedThing":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ReferencedThing property results in inconsistent ThingReference.");
                            result = false;
                        }
                        break;
                }
