...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="ContractChangeNotice"/> class.
    /// </summary>
    [DataContract]
    [CDPVersion("1.1.0")]
    ...
    public partial class ContractChangeNotice : ModellingAnnotationItem
    {
        ...
        /// The revision number.
        /// </param>
        public ContractChangeNotice(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
        }

        /// <summary>
        /// Gets or sets the unique identifier of the referenced ChangeProposal.
        /// </summary>
        [UmlInformation(aggregation: AggregationKind.None, isDerived: false, isOrdered: false, isNullable: false, isPersistent: true)]
        [DataMember]
        public Guid ChangeProposal { get; set; }

        /// <summary>
        /// Gets the route for the current <see ref="ContractChangeNotice"/>.
        /// </summary>
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            {
                dictionary.Add("ChangeProposal", new [] { this.ChangeProposal });
            }
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "Author":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Author property results in inconsistent ContractChangeNotice.");
                                    result = false;
                                }
                                break;
                            ...
                            case "ChangeProposal":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from ChangeProposal property results in inconsistent ContractChangeNotice.");
                                    result = false;
                                }
                                break;
                            ...
                            case "Owner":
                                if (addModelErrors)
                                {
                                    errors.Add($"Removed reference '{id}' from Owner property results in inconsistent ContractChangeNotice.");
                                    result = false;
                                }
                                break;
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Author":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Author property results in inconsistent ContractChangeNotice.");
                            result = false;
                        }
                        break;
                    ...
                    case "ChangeProposal":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ChangeProposal property results in inconsistent ContractChangeNotice.");
                            result = false;
                        }
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ContractChangeNotice.");
                            result = false;
                        }
                        break;
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.ReportingData.ContractChangeNotice"/> from a <see cref="ContractChangeNotice"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.ReportingData.ContractChangeNotice"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.ReportingData.ContractChangeNotice(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as ContractChangeNotice;
            if (original == null)
            {
            ...
            var copyChangeProposal = originalCopyMap.SingleOrDefault(kvp => kvp.Key.Iid == original.ChangeProposal);
            this.ChangeProposal = copyChangeProposal.Value == null ? original.ChangeProposal : copyChangeProposal.Value.Iid;
