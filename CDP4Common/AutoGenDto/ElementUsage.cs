// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ElementUsage.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    public partial class ElementUsage : ElementBase, IOptionDependentThing
    {
        ...
        public override IDictionary<string, IEnumerable<Guid>> GetReferenceProperties()
        {
            ...
            dictionary.Add("ParameterOverride", this.ParameterOverride);
                    ...
                    foreach (var kvp in foundProperty)
                    {
                        switch (kvp.Key)
                        {
                            ...
                            case "ParameterOverride":
                                this.ParameterOverride.Remove(id);
                                break;
                        }
                    }
                }
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "ElementDefinition":
                        ...
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from ElementDefinition property results in inconsistent ElementUsage.");
                            result = false;
                        }
                        break;
                    ...
                    case "ExcludeOption":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ExcludeOption.Remove(toBeRemoved);
                        } 
                        break;
                    ...
                    case "Owner":
                        if (referencedProperty.Value.Except(ids).Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Owner property results in inconsistent ElementUsage.");
                            result = false;
                        }
                        break;
                    ...
                    case "ParameterOverride":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.ParameterOverride.Remove(toBeRemoved);
                        } 
                        break;
                }
            }
