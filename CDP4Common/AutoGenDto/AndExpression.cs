// --------------------------------------------------------------------------------------------------------------------
// <copyright file="AndExpression.cs" company="Starion Group S.A.">
...
namespace CDP4Common.DTO
{
    ...
    /// <summary>
    /// A Data Transfer Object representation of the <see cref="AndExpression"/> class.
    /// </summary>
    [DataContract]
    [Container(typeof(ParametricConstraint), "Expression")]
    public partial class AndExpression : BooleanExpression
    {
        /// <summary>
        ...
        /// The revision number.
        /// </param>
        public AndExpression(Guid iid, int rev) : base(iid: iid, rev: rev)
        {
            this.Term = new List<Guid>();
        }
        ...
        /// <summary>
        /// Gets the route for the current <see ref="AndExpression"/>.
        /// </summary>
            ...
            foreach (var referencedProperty in this.GetReferenceProperties())
            {
                switch (referencedProperty.Key)
                {
                    ...
                    case "Term":
                        foreach (var toBeRemoved in referencedProperty.Value.Except(ids).ToList())
                        {
                            this.Term.Remove(toBeRemoved);
                        } 
                        if (!this.Term.Any())
                        {
                            errors.Add($"Removed reference '{referencedProperty.Key}' from Term property results in inconsistent AndExpression.");
                            result = false;
                        }
                        break;
                }
        ...
        /// <summary>
        /// Instantiate a <see cref="CDP4Common.EngineeringModelData.AndExpression"/> from a <see cref="AndExpression"/>
        /// </summary>
        /// <param name="cache">The cache that stores all the <see cref="CommonData.Thing"/>s</param>.
        /// <param name="uri">The <see cref="Uri"/> of the <see cref="CDP4Common.EngineeringModelData.AndExpression"/></param>.
        /// <returns>A new <see cref="CommonData.Thing"/></returns>
        public override CommonData.Thing InstantiatePoco(ConcurrentDictionary<CacheKey, Lazy<CommonData.Thing>> cache, Uri uri)
        {
            return new CDP4Common.EngineeringModelData.AndExpression(this.Iid, cache, uri);
        }
        ...
        {
            var original = originalThing as AndExpression;
            if (original == null)
            {
